 ...
 * Walker class
 */
class Jet_Blocks_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		// Default class.
		$classes     = array( 'jet-nav__sub' );
		$classes[]   = 'jet-nav-depth-' . $depth;
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$classes[] = 'jet-nav__item-' . $item->ID;
		$classes[] = 'jet-nav__item';
		if ( 0 < $depth ) {
			$classes[] = 'jet-nav-item-sub';
		}
		...
		$show_desc = isset( $args->widget_settings['show_items_desc'] ) ? filter_var( $args->widget_settings['show_items_desc'], FILTER_VALIDATE_BOOLEAN ) : true;

		if ( ! empty( $item->description ) && $show_desc ) {
			$desc = sprintf(
				'<span class="jet-nav-item-desc %2$s">%1$s</span>',
				$item->description,
				( 0 === $depth ) ? 'top-level-desc'  : 'sub-level-desc'
			);
		}

		$title = sprintf( '<span class="jet-nav-link-text">%s%s</span>', $title, $desc );
