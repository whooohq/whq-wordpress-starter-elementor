	...
	protected function make_radio_checkbox_field( $item, $item_index, $type, $form_id, $terms_select = false ) {
		...
		if ( $options ) {
			...
			foreach ( $options as $key => $option ) {
				...
				$html .= '<span class="piotnetforms-field-option"><input ' . $this->get_render_attribute_string( $element_id ) . '> <label for="' . $html_id . '" class="' . $icon_on_top_class . '">' . $icon_html . $option_label . '</label></span>';
			}
