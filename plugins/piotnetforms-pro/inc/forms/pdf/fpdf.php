...
define('tFPDF_VERSION_Piotnetforms','1.31');

class tFPDF_Piotnetforms
{
...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
		if (defined("_SYSTEM_TTFONTS") && file_exists(_SYSTEM_TTFONTS.$file )) { $ttffilename = _SYSTEM_TTFONTS.$file ; }
			...
			$pdf_font_dir = wp_upload_dir()['basedir'].'/'.$file;
			if(file_exists($pdf_font_dir)){
				$ttffilename = $pdf_font_dir;
				$font_name = substr($file, strrpos($file, '/')+1);
				$font_name_lower = strtolower(substr($font_name ,0,(strpos($font_name ,'.'))));
				$unifilename = str_replace($font_name, $font_name_lower, $file);
			}else{
...
{
	$this->metadata['Producer'] = 'tFPDF '.tFPDF_VERSION_Piotnetforms;
