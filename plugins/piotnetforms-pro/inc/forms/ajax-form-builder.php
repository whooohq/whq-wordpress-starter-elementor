	...
	function piotnetforms_check_conditional_logic_for_actions( $fields = [], $actions = [], $conditional_list = [] ) {
		...
		if (!empty($fields) && !empty($actions) && !empty($conditional_list)) {
			...
			foreach ($conditional_list as $item) {
				...
				$conditional_results[$action][] = array(
	...
	function piotnetforms_ajax_form_builder() {
		...
		global $wpdb;
			if ( !empty($_POST['post_id']) && !empty($_POST['form_id']) && !empty($_POST['fields']) ) {
				...
				foreach ($fields as $key_field=>$field) {
					if ($fields[$key_field]['type'] == 'honeypot') {
						if (!empty($fields[$key_field]['value'])) {
							$recaptcha_check = 0;
						}
					}
				...
				if ($recaptcha_check == 1) {
					...
					if (empty($form['settings']['piotnetforms_database_disable'])) {
						...
						if (!empty($form_database_post_id)) {
							...
							$fields_database['form_id_piotnetforms'] = array(
							...
							foreach ($repeater as $repeater_item) {
								$fields_database[$repeater_item['repeater_id']] = array(
									...
									'value' => piotnetforms_get_field_value( '[repeater id="' . $repeater_item['repeater_id'] . '"]', $fields ),
...
', $field['value']) ),
							...
							if($form['settings']['piotnetforms_database_hidden_field'] == 'yes' && !empty($form['settings']['piotnetforms_database_list_field_hidden'])){
									...
									if ($meta_type == 'repeater') {
										...
										if (!empty($custom_field_value)) {
											...
											array_walk($custom_field_value, function (& $item) {
												foreach ($item as $key => $value) {
													$field_object = get_field_object(acf_get_field_key_piotnetforms( $key, $submit_post_id ));
									...
									//if (!empty($custom_field_value)) {
										...
										//PODS
											...
											if ($meta_type == 'date') {
												...
												if (empty($custom_field_value)) {
													$custom_field_value = '';
												} else if (strlen($custom_field_value) > 10) {
										...
										//META BOX
											...
											// 	$custom_field_value = array('address' => $custom_field_value_array['value'], 'lat' => $custom_field_value_array['lat'], 'lng' => $custom_field_value_array['lng'], 'zoom' => $custom_field_value_array['zoom']);
											// 	echo var_dump($custom_field_value);
											// } //TODO GOOGLE MAPS
					...
					if (!empty($form['settings']['piotnetforms_google_sheets_connector_enable']) && !empty($form['settings']['piotnetforms_google_sheets_connector_field_list']) && !empty($form['settings']['piotnetforms_google_sheets_connector_id'])) {
						...
						$google_sheet_expired_token = get_option('piotnetforms-google-sheet-expired-token');
						...
						if ($google_sheet_expired_token < $google_sheet_current_time) {
							...
							if (!empty($google_sheets->access_token)) {
								...
								$gg_sheet_newexpired = get_option('piotnetforms-google-sheet-exprires');
								$gg_sheet_newexpired = (int)$gg_sheet_newexpired;

								update_option( 'piotnetforms-google-sheet-access-token', $gs_atok );

								$google_gheet_new_expired_token = time() + $gg_sheet_newexpired;

								update_option( 'piotnetforms-google-calendar-expired-token', $google_gheet_new_expired_token );
							}
						}

						$gs_atok = get_option('piotnetforms-google-sheet-access-token');
					...
					if (in_array("mailchimp_v3", $form['settings']['submit_actions'])) {
						...
						if($mailchimp_acceptance){
							...
							if(!empty($list_id)){
								...
								$mailchimp_data['status'] = 'subscribed';
								if(!empty($mailchimp_data['merge_fields']['ADDRESS'])){
									if(empty($mailchimp_data['merge_fields']['ADDRESS']['country']) || empty($mailchimp_data['merge_fields']['ADDRESS']['zip']) || $mailchimp_data['merge_fields']['ADDRESS']['state'] || $mailchimp_data['merge_fields']['ADDRESS']['city']){
					...
					if (in_array("pdfgenerator", $form['settings']['submit_actions'])) {
						...
						$pdf_color = str_replace('rgba(', '', $pdf_color);
						$pdf_color = str_replace(', 1)', '', $pdf_color);
						$pdf_color = explode(',', $pdf_color);
						...
						if($form['settings']['pdfgenerator_set_custom']=='yes'){
							foreach($pdf_generator_list as $item){
								...
								if($type == 'image'){
									$pdf_image_url = !empty(replace_email_piotnetforms($item['pdfgenerator_field_shortcode'],$fields,$payment_status, $payment_id)) ? replace_email_piotnetforms($item['pdfgenerator_field_shortcode'],$fields,$payment_status, $payment_id) : plugins_url().'/piotnetforms/assets/images/signature.png';
									$pdf->Image($pdf_image_url, $item_image_x, $item_image_y, $image_width, $image_height);
								}elseif($type == 'image-upload'){
									$pdf_image_url = !empty($item['pdfgenerator_image_field']['url']) ? $item['pdfgenerator_image_field']['url'] : plugins_url().'/piotnetforms/assets/images/signature.png';
									...
									if($item['auto_position'] == 'yes'){
										...
										$pdf->WriteHTML($item_width,$pdf_txt);
									}else{
										$pdf_txt = replace_email_piotnetforms($item['pdfgenerator_field_shortcode'],$fields,$payment_status, $payment_id);
										if(!empty($form['settings']['remove_empty_form_input_fields'])){
						...
						if($form['settings']['pdfgenerator_custom_file_name'] == 'yes' && !empty($form['settings']['pdfgenerator_export_file_name'])){
							$pdf_file_name = $form['settings']['pdfgenerator_export_file_name'];
						}else{
		    ...
		    //     		if (!empty($form['settings']['remote_request_header_list'])) {
						...
						$res = wp_remote_request(replace_email_piotnetforms($form['settings']['remote_request_url'], $fields), $wp_args);
							...
							//} else {
								if (!empty($register_email) && !empty($register_username) && !empty($register_password)) {
									...
									if (is_wp_error($register_user)){ // if there was an error creating a new user
										...
										}else if(!empty($register_user->errors['existing_user_email'])){
									...
									if (!empty($user_meta['update_user_meta']) && !empty($user_meta['update_user_meta_field_shortcode'])) {
										...
										if ($user_meta_key == 'password') {
											...
											if ( $user_meta['update_user_meta'] == 'acf' ) {
												...
												update_user_meta( $user_id, $user_meta_key, piotnetforms_get_field_value($user_meta['update_user_meta_field_shortcode'], $fields) );
											}
										}
									}
								}
	...
	function piotnetforms_process_google_calender($form, $fields, $payment_id) {
		...
		$data_gg_calendar = [
			'summary' => piotnetforms_get_field_value($form['settings']['google_calendar_summary'],$fields, $payment_id),
