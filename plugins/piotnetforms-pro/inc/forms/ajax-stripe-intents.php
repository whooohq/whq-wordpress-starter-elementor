<?php
	...
	add_action( 'wp_ajax_piotnetforms_ajax_stripe_intents', 'piotnetforms_ajax_stripe_intents' );
	add_action( 'wp_ajax_nopriv_piotnetforms_ajax_stripe_intents', 'piotnetforms_ajax_stripe_intents' );

	function find_element_recursive_stripe_piotnetforms( $elements, $form_id ) {
	...
	function set_val_stripe_piotnetforms(&$array,$path,$val) {
	...
	function piotnetforms_merge_string_stripe(&$string,$string_add) {
		$string = $string . $string_add;
	}

	function piotnetforms_unset_string_stripe(&$string) {
		$string = '';
	}

	function piotnetforms_set_string_stripe(&$string,$string_set) {
		$string = $string_set;
	}

	function replace_email_stripe_piotnetforms($content, $fields, $payment_status = 'succeeded', $payment_id = '', $succeeded = 'succeeded', $pending = 'pending', $failed = 'failed', $submit_id = 0 ) {
		$message = $content;
			...
			foreach ($fields as $field) {
				...
				if (!empty($repeater_id) && !empty($repeater_label) && $repeater_remove_this_field == false) {
					if (strpos($repeater_content, $repeater_label) !== false) {
						...
						piotnetforms_merge_string_stripe($repeater_content,$string_add);
					} else {
						$string_add = $repeater_label . $field_label . ': ' . $field_value . '<br />';
						piotnetforms_merge_string_stripe($repeater_content,$string_add);
					}
					if (substr_count($field['repeater_id'],'|') == 2) {
						piotnetforms_set_string_stripe($repeater_id_one,$field['repeater_id_one']);
					}
				}
				...
				if (empty($repeater_id)) {
					if (!empty($repeater_id_one) && !empty($repeater_content)) {
						...
						piotnetforms_unset_string_stripe($repeater_content);
						piotnetforms_unset_string_stripe($repeater_id_one);
					}
				}
	...
	function generatePaymentResponse_piotnetforms($intent,$subscriptions=false) {
	...
	function get_field_name_shortcode_stripe_piotnetforms($content) {
	...
	function piotnetforms_get_field_value_stripe($field_name,$fields, $payment_status = 'succeeded', $payment_id = '', $succeeded = 'succeeded', $pending = 'pending', $failed = 'failed' ) {
		...
		if (strpos($field_name, '[repeater id') !== false) { // ] [ [ fix alert
			...
			$field_name = get_field_name_shortcode_stripe_piotnetforms($field_name);
			$field_value = '';
	...
	function piotnetforms_ajax_stripe_intents() {
		global $wpdb;
		...
		if ( !empty($_POST['post_id']) && !empty($_POST['form_id']) && !empty($_POST['fields']) ) {
			...
			if (!empty($form['settings']['piotnetforms_stripe_customer_field_name'])) {
				$customer_name = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_field_name'], $fields);
				if (!empty($customer_name)) {
					$customer_array['name'] = $customer_name;
				}
			}

			if (!empty($form['settings']['piotnetforms_stripe_customer_field_email'])) {
			...
			if (!empty($form['settings']['piotnetforms_stripe_customer_field_phone'])) {
				$customer_phone = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_field_phone'], $fields);
				if (!empty($customer_phone)) {
					$customer_array['phone'] = $customer_phone;
				}
			}

			if (!empty($form['settings']['piotnetforms_stripe_customer_field_address_line2'])) {
				$customer_address_line2 = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_field_address_line2'], $fields);
				if (!empty($customer_address_line2)) {
					$customer_array['address']['line2'] = $customer_address_line2;
				}
			}

			if (!empty($form['settings']['piotnetforms_stripe_customer_field_address_city'])) {
				$customer_city = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_field_address_city'], $fields);
				if (!empty($customer_city)) {
					$customer_array['address']['city'] = $customer_city;
				}
			}

			if (!empty($form['settings']['piotnetforms_stripe_customer_field_address_country'])) {
				$customer_country = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_field_address_country'], $fields);
				if (!empty($customer_country)) {
					$customer_array['address']['country'] = $customer_country;
				}
			}

			if (!empty($form['settings']['piotnetforms_stripe_customer_field_address_postal_code'])) {
				$customer_postal_code = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_field_address_postal_code'], $fields);
				if (!empty($customer_postal_code)) {
					$customer_array['address']['postal_code'] = $customer_postal_code;
				}
			}

			if (!empty($form['settings']['piotnetforms_stripe_customer_field_address_state'])) {
				$customer_state = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_field_address_state'], $fields);
				if (!empty($customer_state)) {
					$customer_array['address']['state'] = $customer_state;
				}
			}

			if (!empty($form['settings']['piotnetforms_stripe_customer_field_address_line1'])) {
				$customer_address_line1 = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_field_address_line1'], $fields);
			...
			if (empty($form['settings']['piotnetforms_stripe_subscriptions'])) {
				...
				try {
					if (!empty($_POST['payment_method_id'])) {
						...
						$customer_array["description"] = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_info_field'], $fields);
						...
						if (!empty($form['settings']['piotnetforms_stripe_customer_receipt_email'])) {
							$intent_array['receipt_email'] = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_receipt_email'], $fields);
						}

						if (!empty($form['settings']['piotnetforms_stripe_customer_description'])) {
							$intent_array['description'] = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_description'], $fields);
						} else {
							$intent_array['description'] = $form_id;
						}

						$intent = \Stripe\PaymentIntent::create( $intent_array );
						if(!empty($form['settings']['piotnetforms_stripe_create_invoice'])){
							$invoiceitems = [
								'customer' => $customer->id,
								'amount' => $amount,
								'currency' => $currency,
								'description' => replace_email_stripe_piotnetforms($form['settings']['pafe_stripe_customer_description'], $fields),
							];
							$invoice_create = [
								'customer' => $customer->id,
							];
							if(!empty($form['settings']['pafe_stripe_tax_invoice'])){
								$invoice_create['default_tax_rates'] = [replace_email_stripe_piotnetforms($form['settings']['pafe_stripe_tax_invoice'], $fields)];
							}
							piotnetforms_update_invoiceitems(http_build_query($invoiceitems));
							piotnetforms_create_invoice(http_build_query($invoice_create));
						}
					}
					...
					generatePaymentResponse_piotnetforms($intent);
				...
				if (!empty($_POST['payment_method_id'])) {
					...
					$product_id = $form['settings']['piotnetforms_stripe_subscriptions_product_id'];
					$one_time_fee = 0;
					$cancel_after = '';
					if(!empty($form['settings']['piotnetforms_stripe_subscriptions_only_price_enable'])){
						try{
							$subscription_array = array(
							  ...
							  $subscription_array['items'][0]['price'] = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_subscriptions_price_id'], $fields);
							  if(!empty($form['settings']['piotnetforms_stripe_tax_rate_enable']) && !empty($form['settings']['piotnetforms_stripe_tax_rate'])){
									$subscription_array['default_tax_rates'] = [
										replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_tax_rate'], $fields)
									];
								}
							  ...
							  \Stripe\PaymentIntent::update(
								  $intent->id,
								  ['description' => replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_description'], $fields)]
							  );

							  generatePaymentResponse_piotnetforms($intent,true);
						}catch (\Stripe\Exception\ApiErrorException $e) {
							# Display error on client
							echo json_encode([
								'error' => $e->getMessage()
						...
						if (!empty($subscriptions)) {
							if (!empty($product_name)) {
									...
									if (!empty($interval) && !empty($interval_count)) {
										if (!empty($subscriptions[0]['piotnetforms_stripe_subscriptions_amount_field_enable'])) {
											if (!empty($subscriptions[0]['piotnetforms_stripe_subscriptions_amount_field'])) {
												$amount = floatval( piotnetforms_get_field_value_stripe($subscriptions[0]['piotnetforms_stripe_subscriptions_amount_field'], $fields) );
											}
										} else {
											if (!empty($subscriptions[0]['piotnetforms_stripe_subscriptions_amount'])) {
												$amount = floatval( $subscriptions[0]['piotnetforms_stripe_subscriptions_amount'] );
											}
										}
	
										if (!empty($subscriptions[0]['piotnetforms_stripe_subscriptions_one_time_fee'])) {
											$one_time_fee = floatval( $subscriptions[0]['piotnetforms_stripe_subscriptions_one_time_fee'] );
										}
	
										if (!empty($subscriptions[0]['piotnetforms_stripe_subscriptions_cancel'])) {
											if (!empty($subscriptions[0]['piotnetforms_stripe_subscriptions_cancel_add'])) {
												$cancel_after = '+ ' . $subscriptions[0]['piotnetforms_stripe_subscriptions_cancel_add'] . $subscriptions[0]['piotnetforms_stripe_subscriptions_cancel_add_unit'];
											}
										}
									}
								} else {
									if (!empty($form['settings']['piotnetforms_stripe_subscriptions_field_enable'])) {
										$plan_value = piotnetforms_get_field_value_stripe($form['settings']['piotnetforms_stripe_subscriptions_field'], $fields);
										if (!empty($plan_value)) {
														...
														if (!empty($interval) && !empty($interval_count)) {
															if (!empty($subscription_item['piotnetforms_stripe_subscriptions_amount_field_enable'])) {
																if (!empty($subscription_item['piotnetforms_stripe_subscriptions_amount_field'])) {
																	$amount = floatval( piotnetforms_get_field_value_stripe($subscription_item['piotnetforms_stripe_subscriptions_amount_field'], $fields) );
																}
															} else {
																if (!empty($subscription_item['piotnetforms_stripe_subscriptions_amount'])) {
																	$amount = floatval( $subscription_item['piotnetforms_stripe_subscriptions_amount'] );
																}
															}
	
															if (!empty($subscription_item['piotnetforms_stripe_subscriptions_one_time_fee'])) {
																$one_time_fee = floatval( $subscription_item['piotnetforms_stripe_subscriptions_one_time_fee'] );
															}
	
															if (!empty($subscription_item['piotnetforms_stripe_subscriptions_cancel'])) {
																if (!empty($subscription_item['piotnetforms_stripe_subscriptions_cancel_add'])) {
																	$cancel_after = '+ ' . $subscription_item['piotnetforms_stripe_subscriptions_cancel_add'] . $subscription_item['piotnetforms_stripe_subscriptions_cancel_add_unit'];
																}
															}
														}
													}
								...
								if (!empty($amount) && !empty($interval) && !empty($interval_count)) {
									...
									if (!empty($form['settings']['piotnetforms_stripe_subscriptions_product_id'])) {
										$plan_array['product'] = $form['settings']['piotnetforms_stripe_subscriptions_product_id'];
									}
	
									// if (!empty($form['settings']['piotnetforms_stripe_customer_description'])) {
									// 	$plan_array['description'] = replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_description'], $fields);
									// } else {
									// 	$plan_array['description'] = $form_id;
									// }
				...
				if (!empty($_POST['payment_intent_id'])) {
					try {
						...
						if (!empty($form['settings']['piotnetforms_stripe_customer_receipt_email'])) {
							\Stripe\PaymentIntent::update(
								$_POST['payment_intent_id'],
								['receipt_email' => replace_email_stripe_piotnetforms($form['settings']['piotnetforms_stripe_customer_receipt_email'], $fields)]
							);
						}
	...
	function piotnetforms_update_invoiceitems($data){
	...
	function piotnetforms_create_invoice($data){
		$curl = curl_init();
		curl_setopt_array($curl, array(
