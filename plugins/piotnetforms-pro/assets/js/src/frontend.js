...
jQuery(document).ready(function($) {
    ...
    window.piotnetformsValidateFields = function ($fields) {
    	...
    	$fields.each(function(){
			if ( $(this).data('piotnetforms-stripe') == undefined && $(this).data('piotnetforms-html') == undefined ) {
				...
				if ( !$(this)[0].checkValidity() && $(this).closest('.piotnetforms-conditional-logic-hidden').length == 0 && $(this).closest('[data-piotnetforms-conditional-logic]').css('display') != 'none' && $(this).data('piotnetforms-honeypot') == undefined &&  $(this).closest('[data-piotnetforms-signature]').length == 0 || checked == 0 && $checkboxRequired.length > 0 && $(this).closest('.piotnetforms-fields-wrapper').css('display') != 'none') {
			...
			$fields.each(function(){
				if ( $(this).data('piotnetforms-stripe') == undefined && $(this).data('piotnetforms-html') == undefined ) {
					...
					if($(this).attr('data-piotnetforms-is-repassword') != undefined && $(this).attr('data-piotnetforms-is-repassword') != ''){
						var passwordCompare = $(this).closest('.piotnetforms-container').find('[name="form_fields['+$(this).attr('data-piotnetforms-is-repassword')+']"]').val();
	...
	$(document).on('change paste', '[type="number"][data-piotnetforms-id]',function(e){
		var val = $(this).val();
	...
	function piotnetformsLivePreview(field) {
		if ($(document).find('[data-piotnetforms-live-preview]').length > 0) {
			...
			if (fieldId !== undefined) {
				var $livePreview = $(document).find('[data-piotnetforms-live-preview="' + fieldId.replace('form-field-', '') + '"]');
