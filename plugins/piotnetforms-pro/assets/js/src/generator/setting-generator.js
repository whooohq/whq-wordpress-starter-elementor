...
export default class SettingGenerator {
    ...
    generateSettings( $parent ) {
        ...
        if (data_piotnet_editor_widgets_item) {
            const widget_info = JSON.parse(data_piotnet_editor_widgets_item);
            var widget_type = widget_info['type'];
        } else {
            var widget_type = widget_id; // Global Style
        }

        const fieldsObj = {"postID": postID, "fields": {}, 'type': widget_type};
    ...
    generateSettingsField( $fieldGroup, fieldsObj ) {
        ...
        $fieldGroup.each( function() {
            ...
            if ( ($(this).hasClass('hidden') || $(this).closest('.hidden').length > 0) && !is_repeater_field ) {
                return;
            }
