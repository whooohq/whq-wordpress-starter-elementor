jQuery(document).ready(function($) {	
	...
	function initWidgetStripe($scope, $) {
		...
		$.each($elements, function (i, $element) {
			...
			let stripeFontFamily = $(this).attr('data-piotnetforms-form-builder-stripe-font-family');
			...
			let stripeCustomStyle =  $(this).attr('data-piotnetforms-stripe-custom-style');
			if(stripeCustomStyle){
				var style = JSON.parse(stripeCustomStyle);
			}else{
				let stripeStyle = $(this).attr('data-piotnetforms-stripe-style');
				...
				if ( $(this).data('piotnetforms-stripe-submit') != undefined ) {
					...
					$fields.each(function(){
						if ( $(this).data('piotnetforms-stripe') == undefined && $(this).data('piotnetforms-html') == undefined ) {
							...
							if ( !isValid && $(this).closest('.piotnetforms-conditional-logic-hidden').length == 0 && $(this).closest('[data-piotnetforms-conditional-logic]').css('display') != 'none' &&  $(this).closest('[data-piotnetforms-signature]').length == 0 || checked == 0 && $checkboxRequired.length > 0 && $(this).closest('.piotnetforms-fields-wrapper').css('display') != 'none') {
								if ($(this).css('display') == 'none' || $(this).data('piotnetforms-image-select') != undefined || $checkboxRequired.length > 0) {
		      ...
		      if (result.error) {
		        ...
		        $.ajax({
					...
					success: function (confirmResult) {
			        	...
			        	handleServerResponse(JSON.parse(confirmResult), post_id, form_id, fields, amount, stripe, formData, parent); 
			        }
		        }); 
		      }
		    });
					...
					if (response.indexOf(',') !== -1) {
			        	...
			        	if (responseArray.payment_id != '') {
							...
							if (responseArray.register_message != '') {
				        		$parent.find('.piotnetforms-alert--mail .piotnetforms-message-danger').html(responseArray.register_message.replace(/###/g, ','));
				        	}
			        	}
