...
jQuery(document).ready(function($) {
	...
	function piotnetformsCalculatedFieldsForm(fieldNameElement) {
		if (fieldNameElement === '' || window['calculated-data'] === null || window['calculated-data'] === undefined) {
			window['calculated-data'] = {} // clear calculated-data
		}
		const calculated_data = window['calculated-data'];
        ...
        $(document).find(selector).each(function(){
            ...
            if (calculation.indexOf('field id') == -1) {
	            ...
	            // Loop qua tat ca field trong form
	            $(document).find('[name^="form_fields"][data-piotnetforms-id="' + formID + '"]').each(function(field_index, field_dom){
	                if ($(this).attr('id') != undefined) {
	                    var fieldName = $(this).attr('name').replace('[]','').replace('form_fields[','').replace(']',''),
	                    ...
	                    if($fieldSelector.length > 0) {
	                        ...
	                        if (fieldType === 'radio' || fieldType === 'checkbox') {
	                        	if ($repeater_field.length > 0) {
		                        	fieldValue = $(field_dom.closest('[data-piotnetforms-repeater-id]')).find('[data-piotnetforms-id="' + formID + '"][name="form_fields[' + fieldName + ']"]:checked').val();
	                        	} else {
	                        ...
	                        if ($repeater_field.length > 0) {
								...
								calculated_data['piotnetforms_'+fieldName+'_piotnetpiotnetforms'+repeaterIndex+'x'] = fieldValue;
	                        } else {
	                    ...
	                    if (fieldName.indexOf('[]') !== -1) {
	                        ...
	                        if($fieldSelectorMultiple.length > 0) {
	                            ...
	                            if ($repeater_field.length > 0) {
									...
									calculated_data['piotnetforms_'+fieldName+'_piotnetpiotnetforms'+repeaterIndex+'x'] = fieldValueMultipleTotal;
		                        } else {
		                    ...
		                    if($fieldSelector.length > 1 || $fieldSelector.length == 1 && $fieldSelector.closest('[data-piotnetforms-repeater-id]').length > 0) {
		                    	...
		                    	if ($repeater_field.length > 0) {
			                    	$fieldSelector.each(function(field_index, field_dom){
			                    		if (fieldType == 'radio' || fieldType == 'checkbox') {
				                            var fieldValue = $(field_dom.closest('[data-piotnetforms-repeater-id]')).find('[data-piotnetforms-id="' + formID + '"][name="form_fields[' + fieldName + ']"]:checked').val();
				                        } else {
				                        ...
				                        calculated_data['piotnetforms_'+fieldName+'_piotnetpiotnetforms'+repeaterIndex+'x'] = parseFloatWithRemoveSepChar( fieldValue, separatorsSymbol );
			                    	});
		                    	}
		                    }
        	...
        	if ($(this).attr('data-piotnetforms-calculated-fields-coupon-code') != undefined) {
        		...
        		if ($couponCodeFields.length > 0) {
        			...
        			var code_list = $couponCodeField.attr('data-piotnetforms-coupon-code-list');
			...
			if (repeaterTriggerAction == 'add') {
				...
				if ($repeater.length > 0) {
					...
					if (repeaterLimit == 0 || repeaterLimit > 0 && $repeater.eq($repeater.length - 1).siblings('[data-piotnetforms-repeater-id="' + repeaterID + '"]').length < repeaterLimit - 1) {
						...
						if ($repeater.eq($repeater.length - 1).siblings('[data-piotnetforms-repeater-id="' + repeaterID + '"]').length == repeaterLimit-2) {
							$(this).hide();
						}
				...
				$(document).find('[data-piotnetforms-repeater-id-trigger="' + repeaterID + '"]').show();
    ...
    $(window).on('load', function(){
		$('[data-piotnetforms-repeater-value]').each(function(){
	    	...
	    	if ( repeaterJson.length >= 2 ) {
				$('[data-piotnetforms-repeater-form-id] [data-piotnetforms-repeater-trigger-action="remove"]').show();
			}
