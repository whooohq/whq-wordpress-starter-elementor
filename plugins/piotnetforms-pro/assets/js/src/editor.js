...
jQuery(document).ready(function( $ ) {
	...
	$preview_iframe.on('load', function() {
		...
		function has_condition(conditions, control_name) {
			if (!conditions) {
				return false;
			}
			...
			for (let i = 0; i < length; ++i) {
				...
				if (condition.name === control_name) {
					return true;
				}
			}
			return false;
		...
		function get_dependencies_from_structure(widget_structure, control_name) {
			let dependencies = [];
			for(const tab_index in widget_structure) {
				const tab = widget_structure[tab_index];
				const sections = tab['sections'];
				sections.forEach(function (section) {
					if (has_condition(section.conditions, control_name)) {
						dependencies.push({
							'name': section.name,
							'type': 'section',
					...
					dependencies = dependencies.concat(get_dependencies_from_controls(section.controls, control_name, dependencies));
				});
			}
			return dependencies;
		}

		function get_dependencies_from_controls(controls, control_name) {
			let arr = [];
			for(const control of controls) {
				if (has_condition(control.conditions, control_name)) {
					arr.push({
						'name': control.name,
						'type': 'control',
				...
				if (control.controls) {
					arr = arr.concat(get_dependencies_from_controls(control.controls, control_name, arr));
				}
			}
			return arr;
		}
		...
		// TODO rename
		function scan_conditions_in_controls(controls, dependencies, widget_structure, widget_conditions) {
			for(const control of controls) {
				const control_name = control.name;
				if (!StringUtil.isEmpty(control_name)) {
					dependencies[control.name] = get_dependencies_from_structure(widget_structure, control_name);
					// TODO concat if exists
				}
				if (control.conditions) {
					if (StringUtil.isEmpty(control_name)) {
						console.log('control_name empty when has conditions', control);
					}
					widget_conditions[control_name] = control.conditions;
				}
				if (control.controls) {
					scan_conditions_in_controls(control.controls, dependencies, widget_structure, widget_conditions);
				}
			}
		}

		// TODO rename
		function build_dependencies_and_conditions(widget_structure) {
			const dependencies = {};
			const widget_conditions = {};
			...
			for(const tab_index in widget_structure) {
				...
				sections.forEach(function (section) {
					if (section.conditions) {
						widget_conditions[section.name] = section.conditions;
					}
					scan_conditions_in_controls(section.controls, dependencies, widget_structure, widget_conditions);
				});
			}

			let checked = {};
			for(const control_name in dependencies) {
				if (!checked[control_name]) {
					rebuild_dependencies(dependencies, control_name, checked);
				}
			}
			for(const control_name in dependencies) {
				if (dependencies[control_name].length === 0) {
					delete dependencies[control_name];
				}
			}

			checked = {};
			for(const control_name in widget_conditions) {
				if (!checked[control_name]) {
					rebuild_conditions(widget_conditions, control_name, checked);
				}
			}

			return {dependencies, conditions: widget_conditions};
		}

		function append_dependencies(dependence_conditions, control_name, other_dependencies) {
			for (const other_dependence of other_dependencies) {
				let found = false;
				for (const dependence of dependence_conditions[control_name]) {
					if (other_dependence.name === dependence.name && other_dependence.type === dependence.type) {
						found = true;
						break;
					}
				}
				if (!found) {
					dependence_conditions[control_name].push(other_dependence);
				}
			}
		}

		function rebuild_dependencies(dependence_conditions, control_name, checked) {
			const dependencies = dependence_conditions[control_name];
			for (const dependence of dependencies) {
				const name = dependence.name;
				if (dependence_conditions[name] && dependence_conditions[name].length > 0) {
					if (!checked[name]) {
						rebuild_dependencies(dependence_conditions, name, checked);
					}
					append_dependencies(dependence_conditions, control_name, dependence_conditions[name]);
				}
			}
			checked[control_name] = true;
		}

		function rebuild_conditions(widget_conditions, control_name, checked) {
			for (const condition of widget_conditions[control_name]) {
				if (widget_conditions[condition.name]) {
					if (!checked[condition.name]) {
						rebuild_dependencies(widget_conditions, condition.name, checked);
					}
					widget_conditions[control_name] = widget_conditions[condition.name].concat(widget_conditions[control_name]);
				}
			}
			checked[control_name] = true;
		}

		function build_style_controls(widget_type, widget_structure) {
			const style_controls = [];
			for(const tab_index in widget_structure) {
				const tab = widget_structure[tab_index];
				const sections = tab['sections'];
				sections.forEach(function (section) {
					scan_style_in_controls(section.controls, style_controls);
				});
			}

			// const dependence_controls = []
			// const dependence_conditions = pb.get_dependencies(widget_type).conditions;
			//
			// for (const style_control of style_controls) {
			// 	const conditions = dependence_conditions[style_control];
			// 	if (conditions) {
			// 		for (const condition of conditions) {
			// 			const condition_name = condition['name'];
			// 			if (dependence_controls.indexOf(condition_name) < 0) {
			// 				if (condition_name == 'field_label') {
			// 					console.log('style_control', style_control);
			// 				}
			// 				dependence_controls.push(condition_name);
			// 			}
			// 		}
			// 	}
			// }

			// for (const dependence_control of dependence_controls) {
			// 	if (style_controls.indexOf(dependence_control) < 0) {
			//
			// 		style_controls.push(dependence_control);
			// 	}
			// }
		...
		function scan_style_in_controls(controls, style_controls) {
			for(const control of controls) {
				const control_name = control.name;
				...
				if (!StringUtil.isEmpty(control_name) && control.selectors && style_controls.indexOf(control_name) < 0) {
					style_controls.push(control_name);
				}

				if (control.controls) {
					scan_style_in_controls(control.controls, style_controls);
				}
			}
		}
		...
		function initialize_editor() {
			...
			for (const widget_type in widget_infos) {
				const structure = widget_infos[widget_type].structure;
				const dependencies = build_dependencies_and_conditions(structure);
				pb.set_dependencies(widget_type, dependencies);

				const style_controls = build_style_controls(widget_type, structure);
				pb.set_style_controls(widget_type, style_controls);
			}
			...
			get_libs(get_lib_keys_from_widget_infos(widget_infos)).then(libs => {
				...
				const raw_global_settings = $('[data-piotnetforms-global-style]').val();
				const global_settings = raw_global_settings !== '' ? JSON.parse(raw_global_settings) : {};

				const setting_widgets = data['widgets'] ? data['widgets'] : {};
				...
				for (const key in global_settings) {
					pb.master.global.push(key);
					setting_widgets[key] = global_settings[key];
					if (setting_widgets[key]['fields']) {
						if (setting_widgets[key]['fields'].length == 0) {
							setting_widgets[key]['fields'] = {};
						}
					}
				}
				...
				for (let widget_id in setting_widgets) {
					const widget_structure = pb.build_widget_structure(widget_id);
					pb.set_widget_structure(widget_id, widget_structure);
				}
				...
				$els.each(function(){
					...
					const $template = _.template($(this).html());
		...
		function generate_css_from_control(control, widget_id, control_value, repeater_id) {
			...
			if (selectors && !is_control_empty_value(control_type, control_value)) {
				for (const selector_key in selectors) {
					...
					wrapper = StringUtil.replaceAll(wrapper, '{{WRAPPER_EDITOR}}', '#piotnetforms [data-piotnet-editor-widgets-item-id="' + widget_id + '"]');
					wrapper = StringUtil.replaceAll(wrapper, '{{GLOBAL}}', '#piotnetforms ');
					if (repeater_id) {
		...
		$(document).on('change','[data-piotnet-control-dimensions]', function(e) {
			var $control_dimensions = $(this).closest('[data-piotnet-control-dimensions-name]'),
				$control_dimensions_item = $control_dimensions.find('[data-piotnetforms-settings-field]'),
				$control_dimensions_isLinked = $control_dimensions.find('[data-piotnet-control-dimensions="isLinked"]');

			if ($(this).attr('data-piotnet-control-dimensions') == 'isLinked') {
				if ($(this).prop("checked") === true) {
					$control_dimensions_item.val( $control_dimensions.find('[data-piotnet-control-dimensions="top"]').val() ).change();
				} else {
					$control_dimensions_item.each(function() {
						if($(this).val() === '') {
							$(this).val(0).change();
						}
					});
				...
				if ($control_dimensions_isLinked.prop("checked") !== true) {
					if($(this).val() === '') {
						$(this).val(0).change();
					}
				}
		...
		$(document).on('click', '[data-piotnet-control-responsive]', function(){
			...
			if (responsive == 'mobile') {
				$('[data-piotnetforms-preview-inner]').css({'width': '400px', 'flex': 'none'});
		...
		$(document).on('click', '[data-piotnet-control-gallery-change-image]', function(e) {
			...
			file_frame.on( 'select', function() {
				...
				$galleryItem.find('[data-piotnet-control-gallery="preview"]').attr('src', attachment.url);
			});
		...
		function resizableWidget() {
			$iframe.find('[data-piotnet-editor-widgets-item]').not('[data-piotnet-editor-column]').not('[data-piotnet-editor-section]').resizable({
				handles: 'e',
		        start: function(event, ui){
		        	var $container = $(this).parent('div');
		            $iframe.find('[data-piotnet-editor-widgets-item-id]').not(this).removeClass('active');
	            	$(this).addClass('active');
	            	$(this).append('<div class="piotnetforms-resizable-width"></div>');
	            	$container.addClass('piotnetforms-highlight');
	            	$container.append('<div class="piotnetforms-highlight__placeholder" style="width:' + $container.width() + 'px; height:' + $container.height() + 'px; top:' + $container.css('padding-top') + '; left:' + $container.css('padding-left') + ';"></div>');
		        },
		        resize: function(event, ui){
		        	var $container = $(this).parent('div'),
		            	cellPercentWidth = Math.round( 100 * ui.originalElement.outerWidth() / $container.width() * 1000 ) / 1000;

	            	$(this).find('.piotnetforms-resizable-width').html(cellPercentWidth + '%');
		        },
		        stop: function(event, ui){
		        	$(this).find('.piotnetforms-resizable-width').remove();

		        	var $container = $(this).parent('div'),
		            	cellPercentWidth = Math.round( 100 * ui.originalElement.outerWidth() / $container.width() * 1000 ) / 1000,
		            	widget_id = $(this).attr('data-piotnet-editor-widgets-item-id'),
		            	widget_settings = pb.get_setting_widget(widget_id),
		            	current_device = ( $(document).find('[data-piotnet-control-responsive].active').length > 0 ) ? $(document).find('[data-piotnet-control-responsive].active').attr('data-piotnet-control-responsive') : 'desktop',
		            	advanced_width = 'advanced_width_responsive_' + current_device,
		            	$panelWidgetSettings = $("[data-piotnetforms-editor-widget-settings]");

	            	if (cellPercentWidth > 100) {
	            		cellPercentWidth = 100;
	            	}

	            	$iframe.find('.piotnetforms-highlight__placeholder').remove();
	            	$iframe.find('.piotnetforms-highlight').removeClass('piotnetforms-highlight');
	            	widget_settings['fields'][advanced_width] = cellPercentWidth;
	            	pb.set_setting_widget(widget_id, widget_settings);
	            	click_edit_widget($(this).find('[data-piotnet-control-edit]').eq(0), advanced_width, cellPercentWidth);
					$panelWidgetSettings.find('[data-piotnetforms-widget-controls="' + widget_id + '"] [name="' + advanced_width + '"]').val(cellPercentWidth).change();
		...
		$iframe.bind("contextmenu", function (event) {

		    // Avoid the real one
		    event.preventDefault();

		    if ($(event.target).closest('[data-piotnet-editor-widgets-item-id]').length > 0) {
		    	window['piotnetforms_contextmenu_target_widget_id'] = $(event.target).closest('[data-piotnet-editor-widgets-item-id]').attr('data-piotnet-editor-widgets-item-id');
		    }

		    // Show contextmenu
		    $iframe.find("[data-piotnetforms-contextmenu]").show();

		    // In the right position (the mouse)
		    $iframe.find("[data-piotnetforms-contextmenu]").css({
		        top: event.pageY + "px",
		        left: event.pageX + "px"
		    });
		});
		...
		$iframe.bind("mousedown", function (e) {

		    // If the clicked element is not the menu
		    if (!$(e.target).parents("[data-piotnetforms-contextmenu]").length > 0) {

		        // Hide it
		        $iframe.find("[data-piotnetforms-contextmenu]").hide();
		    }
		});


		// If the menu element is clicked
		$iframe.find("[data-piotnetforms-contextmenu-action]").click(function(){
			const target_widget_id = window['piotnetforms_contextmenu_target_widget_id'];
			const COPY_STYLE_KEY = 'copy_style';

		    // This is the triggered action name
		    switch($(this).attr("data-piotnetforms-contextmenu-action")) {

		        // A case for each action. Your actions here
		        case "copy-style":
		        	if (target_widget_id) {
		        		const setting_widget = pb.get_setting_widget( target_widget_id );
		        		if (setting_widget) {
							const copy_style_data = JSON.stringify(pb.get_setting_widget( window['piotnetforms_contextmenu_target_widget_id']));
							localStorage.setItem(COPY_STYLE_KEY, copy_style_data );
						}
		        	}
		        	break;
		        case "paste-style":
					const setting_widget = JSON.parse(localStorage.getItem(COPY_STYLE_KEY));
					if (setting_widget) {
						const setting_widget_type = setting_widget.type;
						const target_setting_widget = pb.get_setting_widget( target_widget_id );
						if (setting_widget_type === target_setting_widget.type) {
							const style_controls = pb.get_style_controls(setting_widget_type);

							const setting_widget_fields = setting_widget.fields;
							const target_setting_widget_fields = target_setting_widget.fields;

							// Copy to setting
							for (const style_control of style_controls) {
								if (style_control in setting_widget_fields) {
									target_setting_widget_fields[style_control] = setting_widget_fields[style_control];
								} else {
									delete target_setting_widget_fields[style_control]
								}
							}

							// Copy to tree setting
							const tree_widget = pb.get_tree_setting_widget(target_widget_id);
							tree_widget.settings = target_setting_widget.fields;

							// Re-generate CSS
							const widgetCss = generate_css_from_settings(target_widget_id);
							pb.set_css_widget(target_widget_id, widgetCss);
							$iframe.find('[data-piotnet-widget-css-head="' + target_widget_id + '"]').html(widgetCss);

							// Detach control in panel and back to Menu
							...
							pb.set_control_widget(target_widget_id, null);

							// Re-generate widget structure
							const widget_structure = pb.build_widget_structure(target_widget_id);
							pb.set_widget_structure(target_widget_id, widget_structure);
						}
					}
		        	break;
		    }
		...
		function reInitFieldsControls($parent) {
			...
			$parent.find('[data-piotnetforms-get-fields]').each(function(){
				$(this).select2({
					tags: true,
					allowClear: true,
					placeholder: '',
		...
		function processNavigatorWhenDropWidget(mainWidgetId, widgetId, tree_widgets) {
			for (let i = 0; i < tree_widgets.length; i++) {
				const widget = tree_widgets[i];
				if (widget['id'] == widgetId) {
					if (tree_widgets.length == 1) {
						// Tìm thấy bên trong chỉ có duy nhất 1 widget
						if (mainWidgetId) {
							const $mainNavigator = $('[data-navigator-piotnetforms-widget-id="' + mainWidgetId + '"]');
							const level = parseInt($mainNavigator.attr('data-navigator-piotnetforms-level'));
							const navigator_html = build_navigator_html(widget, level + 1);
							$mainNavigator.find('.piotnetforms-navigator-sub-widgets').html(navigator_html);
						} else {
							const $mainNavigator = $('.piotnetforms-navigator-widgets');
							const navigator_html = build_navigator_html(widget, 1);
							$mainNavigator.html(navigator_html);
						}
					} else if (i == 0) {
						// Nằm đầu tiên
						const beforeWidgetId = tree_widgets[i + 1]['id']
						const $beforeWidget = $('[data-navigator-piotnetforms-widget-id="' + beforeWidgetId + '"]');
						const level = parseInt($beforeWidget.attr('data-navigator-piotnetforms-level'));

						const navigator_html = build_navigator_html(widget, level);
						$beforeWidget.before(navigator_html);
					} else {
						const afterWidgetId = tree_widgets[i - 1]['id'];
						const $afterWidget = $('[data-navigator-piotnetforms-widget-id="' + afterWidgetId + '"]');
						const level = parseInt($afterWidget.attr('data-navigator-piotnetforms-level'));

						const navigator_html = build_navigator_html(widget, level);
						$afterWidget.after(navigator_html);
					}
					return true;
				}

				const subWidgets = widget['elements'];
				if (subWidgets && subWidgets.length > 0) {
					if (processNavigatorWhenDropWidget(widget['id'], widgetId, subWidgets)) {
						return true;
					}
				}
			}
		...
		function rebuildNavigatorWhenDropWidget(widgetId) {
			if (isNavigatorRendered) {
				const tree_widgets = pb.get_tree_setting_widgets();
				$('[data-navigator-piotnetforms-widget-id="' + widgetId + '"]').remove();
				processNavigatorWhenDropWidget(null, widgetId, tree_widgets);
			}
		}

		function create_new_widget($widget) {
			$widget.attr('style','');
			const form_id = $('[data-piotnetforms-form-title]').attr('data-piotnetforms-form-title');
			...
			if ($widget.attr('data-piotnetforms-editor-widgets-item-panel') === undefined) {
				...
				setTimeout(function () {
					generateWidgetsSettings();
					rebuildNavigatorWhenDropWidget($widget.attr('data-piotnet-editor-widgets-item-id'));
				}, 0);
				return;
			}
			...
			if (widget_type === 'section' || widget_type === 'column') {
				...
				setTimeout(function () {
					...
					rebuildNavigatorWhenDropWidget(widget_id);
				}, 0)

				$($widgetSettings).find('[name="piotnetforms_conditional_logic_form_form_id"]').val(form_id);
				$($widgetSettings).find('[name="piotnetforms_repeater_form_id"]').val(form_id);
			...
			if (widget_type === 'field') {
				...
				$($widgetSettings).find('[name="form_id"]').val(form_id);
				$($widgetSettings).find('.piotnetforms-field-shortcode').val('[field id="' + widget_id + '"]');
				$($widgetSettings).find('[name="live_preview_code"]').val('<span data-piotnetforms-live-preview="' + widget_id + '"></span>');
			} else if (widget_type === 'booking') {
				$($widgetSettings).find('[name="piotnetforms_booking_id"]').val(widget_id);
				$($widgetSettings).find('[name="piotnetforms_booking_form_id"]').val(form_id);
				$($widgetSettings).find('.piotnetforms-field-shortcode').val('[field id="' + widget_id + '"]');
			} else if (widget_type === 'submit') {
				$($widgetSettings).find('[name="form_id"]').val(form_id);
			} else if (widget_type === 'woocommerce-checkout') {
				$($widgetSettings).find('[name="piotnetforms_woocommerce_checkout_form_id"]').val(form_id);
			} else if (widget_type === 'preview-submissions') {
				$($widgetSettings).find('[name="form_id"]').val(form_id);
			} else if (widget_type === 'multi-step-form') {
				$($widgetSettings).find('[name="form_id"]').val(form_id);
			}

			if (widget_type !== 'field' && widget_type !== 'submit') {
				$($widgetSettings).find('[name="piotnetforms_conditional_logic_form_form_id"]').val(form_id);
				$($widgetSettings).find('[name="piotnetforms_repeater_form_id"]').val(form_id);
			}
		...
		function remove_global_settings_edited() {
			$(document).find('[data-piotnetforms-global-settings-edited]').removeAttr('data-piotnetforms-global-settings-edited');
			$iframe.find('[data-piotnetforms-global-settings-edited]').removeAttr('data-piotnetforms-global-settings-edited');
		}

		function back_to_menu() {
			if ($iframe.find('[data-piotnetforms-global-settings-edited]').length > 0 || $(document).find('[data-piotnetforms-global-settings-edited]').length > 0) {
				if (confirm('Please Click Save Button to Save Global Settings')) {
					return;
				} else {
		...
		function click_edit_widget(thiz, controlName, controlValue, widget_id) {
			$(document).find('.select2-hidden-accessible').each(function(){
				$(this).select2('close');
			});

			if ($(thiz).closest('[data-piotnetforms-global-settings-edited]').length > 0) {
				if (confirm('Please Click Save Button to Save Global Settings')) {
					return;
				} else {
					remove_global_settings_edited();
			...
			if ($control_widget_el.length === 0) {
				...
				if (!$control_widget_el) {
					...
					const $repeaterList = $control_widget_el.find('[data-piotnet-control-repeater-list]');
				...
				if (controlName && controlValue) {
					$control_widget_el.find('[name="' + controlName + '"]').val(controlValue);
				}
			...
			$('.piotnetforms-navigator').find('.piotnetforms-navigator-widget').removeClass('active');
			// TODO handle expand to widget

			const $navigatorWidget = $('[data-navigator-piotnetforms-widget-id="' + widget_id + '"]').find('.piotnetforms-navigator-widget').eq(0);
			const $navigatorParent = $navigatorWidget.parents('.piotnetforms-navigator-sub-widgets');
			for (let i = $navigatorParent.length - 1; i >=0; i--) {
				$($navigatorParent[i]).slideDown(0);
				// $($navigatorParent[i]).slideDown(100 * ($navigatorParent.length - i - 1));
			}
			$navigatorWidget.addClass('active');
		}

		$(document).on('click', '[data-piotnet-global-settings]', function(e) {
			click_edit_widget('', '', '', 'form-global');
			$iframe.find('[data-piotnetforms-widget-preview]').attr('data-piotnetforms-global-settings-edited','');
			$('.piotnetforms-editor').attr('data-piotnetforms-global-settings-edited','');
		});
		...
		function duplicate_widget($widget, is_deep_duplicate) {
			...
			$clone_widget.find('.ui-resizable-handle').remove();
			...
			const widget_structure = pb.build_widget_structure(widget_id, setting_widget['fields']);
			pb.set_widget_structure(clone_widget_id, widget_structure);
		...
		$iframe.on('click', '[data-piotnet-control-duplicate]', function(e) {
			...
			// Handle navigator
			if (isNavigatorRendered) {
				const new_widget_id = $($clone_widget[0]).attr('data-piotnet-editor-widgets-item-id');
				var tree_widget = pb.get_tree_setting_widget(new_widget_id);
				var $navigator = $('[data-navigator-piotnetforms-widget-id="' + widget_id + '"]');
				const level = parseInt($navigator.attr('data-navigator-piotnetforms-level'));
				$navigator.after(build_navigator_html(tree_widget, level));
			}
		});
		...
		$('[data-piotnetforms-editor-save]').click(function(e){
			...
			for (const widget_id in css_widgets) {
				let global_widget = pb.master.global.includes(widget_id) ? true : false;
				if (global_widget) {
					global_css += css_widgets[widget_id] + ' ';
				} else {
			...
			for (var i = 0; i < pb.master.global.length; i++) {
				let key = pb.master.global[i];
				global_settings[key] = pb.get_setting_widget(key);
				pb.remove_setting_widget(key);
			}
			...
			if ($(this).closest('[data-piotnetforms-global-settings-edited]').length > 0) {
				data['piotnet-global-css'] = global_css;
				data['piotnetforms_global_settings'] = JSON.stringify(global_settings);
			}

			$.post(ajaxurl, data, function(response) {
				for (var i = 0; i < pb.master.global.length; i++) {
					let key = pb.master.global[i];
					pb.set_setting_widget(key, global_settings[key]);
				}
		...
		$(document).on('keyup change', '[name="field_id"]', function() {
			$(this).closest('[data-piotnet-controls-section]').find('.piotnetforms-field-shortcode').val('[field id="' + $(this).val() + '"]').change();
			$(this).closest('[data-piotnet-controls-section]').find('[name="live_preview_code"]').val('<span data-piotnetforms-live-preview="' + $(this).val() + '"></span>').change();
		});
		...
		$(document).on('select2:opening', '[data-piotnetforms-get-fields]', function() {
			var widgets = pb.master.settings.widgets,
				fields = [],
				fields_shortcode_array = [],
				current_value = $(this).val(),
				widget_id = $(this).closest('[data-piotnetforms-widget-controls]').attr('data-piotnetforms-widget-controls');

			for ( var key in widgets ) {
				if (widgets[key].type == 'field') {
					var field_settings = widgets[key].fields,
						widget_form_id = '';

					if (widgets[widget_id].fields.form_id) {
						widget_form_id = widgets[widget_id].fields.form_id;
					} else if (widgets[widget_id].fields.piotnetforms_booking_form_id) {
						widget_form_id = widgets[widget_id].fields.piotnetforms_booking_form_id;
					} else if (widgets[widget_id].fields.piotnetforms_woocommerce_checkout_form_id) {
						widget_form_id = widgets[widget_id].fields.piotnetforms_woocommerce_checkout_form_id;
					} else if (widgets[widget_id].fields.piotnetforms_conditional_logic_form_form_id) {
						widget_form_id = widgets[widget_id].fields.piotnetforms_conditional_logic_form_form_id;
					}

					if (field_settings.form_id == widget_form_id) {
						var	field_id = field_settings.field_id,
							field_label = field_settings.field_label,
							field_item = { field_id: field_id, field_label: field_label};
						fields.push( field_item );
						fields_shortcode_array.push( '[field id="' + field_id + '"]' );
					}
				}
			}
			...
			if (current_value && !fields_shortcode_array.includes(current_value)) {
				$(this).append($("<option></option>").attr("value",current_value).attr('selected','selected').text(current_value));
			}

			for (var i = 0; i < fields.length; i++) {
				if( widgets[widget_id].fields.field_id !== fields[i].field_id && $(this).attr('data-piotnetforms-get-fields-include-itself') === undefined  || $(this).attr('data-piotnetforms-get-fields-include-itself') !== undefined ) {
					var field_shortcode = '[field id="' + fields[i].field_id + '"]',
						field_label = (fields[i].field_label) ? fields[i].field_label : field_shortcode;
					if (current_value && current_value==field_shortcode) {
						$(this).append($("<option></option>").attr("value",field_shortcode).attr('selected','selected').text(field_label));
					} else {
						$(this).append($("<option></option>").attr("value",field_shortcode).text(field_label));
					}
				}
			}
		});

		$(document).on('select2:closing', '[data-piotnetforms-get-fields]', function() {
			$(this).trigger('change');
		});
		...
		$(document).on('click', '.piotnetforms-field-shortcode', function() {
			...
			$(this).val('[field id="' + fieldId + '"]');
			$(this).change();
		});

		$(document).on('click', '[name="live_preview_code"]', function() {
			const $controls_section_el = $(this).closest('[data-piotnet-controls-section]');
			let fieldId = $controls_section_el.find('[name="field_id"]').val();
			$(this).val('<span data-piotnetforms-live-preview="' + fieldId + '"></span>');
			$(this).change();
		});
		...
		//Mailerlite_V2
		$(document).on('click', '[data-piotnetforms-mailerlite-api-get-groups]', function(){
			$(this).addClass('loading');
			const parent = $(this).closest('[data-piotnet-controls-section-body]');
			var mailerliteApi = parent.find('[name="mailerlite_api_key_source_v2"]').eq(0).val();
			if(mailerliteApi == 'custom'){
				var mailerliteApiKey = parent.find('[name="mailerlite_api_key_v2"]').eq(0).val();
			}else{
				var mailerliteApiKey = false;
			}
			var mailerlite_groups = {
				action: 'piotnetforms_mailerlite_get_groups',
				apiKey: mailerliteApiKey
			}
			$.post(ajaxurl, mailerlite_groups, function(response) {
				if(response){
					$('[data-piotnetforms-mailerlite-api-get-groups-results]').html(response);
				}
			});
			var mailerlite_fields = {
				action: 'piotnetforms_mailerlite_get_fields',
				apiKey: mailerliteApiKey
			}
			$.post(ajaxurl, mailerlite_fields, function(response) {
				if(response){
					$('[data-piotnetforms-mailerlite-api-get-fields-results]').html(response);
					$('[data-piotnetforms-mailerlite-api-get-groups]').removeClass('loading');
				}
			});
		});
		//PDF Auto Load Template
		$(document).on('click', '[name="pdfgenerator_import_template"]', function(){
			if($(this).prop("checked")){
				const body = $(this).closest('[data-piotnet-controls-section-body]');
				let pdf_url = body.find('[name="pdfgenerator_template_url"]').val();
				if(pdf_url){
					setTimeout(
						function(){
							$iframe.find('#piotnetforms-load-pdf-template').trigger('click');
						}, 200
					);	
				}
			}
		});
		$(document).on('change', '[name="pdfgenerator_template_url"]', function(){
			if($(this).val() != ''){
				setTimeout(
					function(){
						$iframe.find('#piotnetforms-load-pdf-template').trigger('click');
		...
		function build_navigator_html(widget, level) {
			const paddingLeft = level * 10;
			const widgetId = widget['id'];

			const widget_info = pb.get_widget_info_by_class_name(widget['class_name'])
			const widgetTitle = widget_info ? widget_info['title'] : widget['class_name'].replace("piotnetforms_", "");

			let mainClass = "piotnetforms-navigator-widget-section";

			const subElements = widget['elements'];
			const hasSubElements = subElements && subElements.length > 0;
			const innerHTMLs = [];
			if (hasSubElements) {
				// mainClass += " piotnetforms-navigator-widget-section-has-sub-widgets";
				for (const subElement of subElements) {
					innerHTMLs.push(build_navigator_html(subElement, level + 1));
				}
			}
			const innerHTMLJoined = innerHTMLs.join("\n");

			// TODO ẩn piotnetforms-navigator-toggle-sub-widgets khi không có sub widgets
			// TODO drag navigator

			let toggleSubWidgetClass = "piotnetforms-navigator-toggle-sub-widgets";
			if (widgetTitle != 'Section' && widgetTitle != 'Column') {
				toggleSubWidgetClass += " piotnet-navigator-hide-toggle-sub-widgets";
			}

			const html = `<div data-navigator-piotnetforms-widget-id="${widgetId}" data-navigator-piotnetforms-level="${level}" class="${mainClass}" style="">
				<div class="piotnetforms-navigator-widget" style="padding-left: ${paddingLeft}px;">
					<div class="${toggleSubWidgetClass}">
						<i class="icon-angle-down"></i>
					</div>
					<div class="piotnetforms-navigator-widget-title">
						<span class="piotnetforms-navigator-widget-title-text">${widgetTitle}</span>
					</div>
				</div>
				<div class="piotnetforms-navigator-sub-widgets" style="display: none;">
					${innerHTMLJoined}
				</div>
			</div>`;
			return html;
		...
		$(document).find('.piotnetforms-navigator').draggable({ handle: '.piotnetforms-navigator-header' });

		let isNavigatorRendered = false;
		$(document).on('click', '[data-piotnetforms-navigator-toggle]', function(e) {
			const $piotnetformsNavigator = $(document).find('.piotnetforms-navigator');
			const hiddenStatusBeforeToggle = $piotnetformsNavigator.hasClass('piotnetforms-navigator-hidden');
			$piotnetformsNavigator.toggleClass('piotnetforms-navigator-hidden');

			if (!hiddenStatusBeforeToggle || isNavigatorRendered) {
				return;
			}

			isNavigatorRendered = true;

			const $navigator = $(document).find('.piotnetforms-navigator-widgets');

			const widgets = pb.get_tree_setting_widgets();

			const htmls = [];
			for (const widget of widgets) {
				htmls.push(build_navigator_html(widget, 1));
			}

			$navigator.html(htmls.join("\n"));
		});

		$(document).on('click', '.piotnetforms-navigator-close', function(e) {
			$(document).find('.piotnetforms-navigator').toggleClass('piotnetforms-navigator-hidden');
		});

		$(document).on('click', '.piotnetforms-navigator-toggle-all', function(e) {
			if ($(this).hasClass("icon-angle-down")) {
				$(this).removeClass("icon-angle-down");
				$(this).addClass("icon-angle-up");
				$(document).find('.piotnetforms-navigator-sub-widgets').slideDown(0);
			} else {
				$(this).removeClass("icon-angle-up");
				$(this).addClass("icon-angle-down")
				$(document).find('.piotnetforms-navigator-sub-widgets').slideUp(0);
			}
		});

		$(document).on('click', '.piotnetforms-navigator-toggle-sub-widgets', function(e) {
			e.stopPropagation();
			e.preventDefault();
			...
			const $navigatorWidgetSection = $(this).closest('.piotnetforms-navigator-widget-section');
			$navigatorWidgetSection.find('.piotnetforms-navigator-sub-widgets').first().slideToggle(0);
		});

		$(document).on('click', '[data-navigator-piotnetforms-widget-id]', function(e) {
			e.stopPropagation();
			e.preventDefault();

			const widgetId = $(this).attr('data-navigator-piotnetforms-widget-id');
			const $widget = $iframe.find('[data-piotnet-editor-widgets-item-id="' + widgetId + '"]')
			if ($widget.length > 0) {
				click_edit_widget($widget);
			}
		});
