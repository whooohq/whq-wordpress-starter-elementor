...
jQuery(document).ready(function($) {
	...
	$(document).on( 'click', '[data-piotnetforms-woocommerce-checkout-place-order]#place_order', function(e){
		var formID = $(this).closest('[data-piotnetforms-woocommerce-checkout-form-id]').attr('data-piotnetforms-woocommerce-checkout-form-id'),
		    $fields = $(document).find('[data-piotnetforms-id="'+ formID +'"]'),
		    error = window.piotnetformsValidateFields($fields);
		if (error != 0) {
			e.preventDefault();
		}
	});
				...
				$.ajax({
					...
					success: function (response) {
						if (response.trim() == '1') {
							...
							$place_order_button.attr('data-piotnetforms-woocommerce-checkout-place-order', '');
						}
					}
				});
			});
