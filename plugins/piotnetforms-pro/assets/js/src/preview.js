...
import './frontend/nice-number.js';
...
jQuery(document).ready(function( $ ) {
	...
	$(document).on('click','[data-piotnetforms-campaign-get-data-list]', function() {
		...
		if(campaign == 'custom'){
			var campaign_url = $parent.find( '[name="activecampaign_api_url"]' ).val();
			var campaign_key = $parent.find( '[name="activecampaign_api_key"]' ).val();
		}else{
			var campaign_url = 'false';
			var campaign_key = 'false';
		}
		var data = {
		...
		$.post(ajaxurl, data, function(response) {
			if(response){
				...
				$parent.find('[name="activecampaign_list"]').trigger('click');
				$(document).find('[data-piotnetforms-campaign-get-data-list]').removeClass('loading');
			}
		});
	});
	//?
	$(document).on('click','[name="activecampaign_list"]', function() {
		var $parent = $(this).closest('[data-piotnet-controls-section-body]');
	...
	//hubspot
	$(document).on('click','[data-piotnetforms-hubspot-get-property-list]', function() {
		$('[data-piotnetforms-hubspot-get-property-list]').addClass('loading');
		let $parent =  $(this).closest('[data-piotnet-controls-section-body]');
		let hubspotGroup = $parent.find( '[name="piotnetforms_hubspot_group_key"]' ).val();
		let data = {
			'action': 'piotnetforms_hubspot_contact_property_list',
			'group' : hubspotGroup,
		};
		$.post(ajaxurl, data, function(response) {
			if(response){
				$('.piotnetforms-hubspot-property-list').html(response);
				$('[data-piotnetforms-hubspot-get-property-list]').removeClass('loading');
			}
		});
	});

	$(document).on('click','[data-piotnetforms-hubspot-get-group-list]', function() {
		$('[data-piotnetforms-hubspot-get-group-list]').addClass('loading');
		var data = {
			'action': 'piotnetforms_hubspot_group_list',
		};
		$.post(ajaxurl, data, function(response) {
			if(response){
				$('.piotnetforms-hubspot-group-list').html(response);
				$('[data-piotnetforms-hubspot-get-group-list]').removeClass('loading');
			}
		});
	});
	...
	//Sendinblue
	$(document).on('click', '[data-piotnetforms-sendinblue-get-list]', function () {
		$(this).addClass('loading');
		let apiKey;
		let parent = $(this).closest('[data-piotnet-controls-section-body]');
		let apiType = parent.find('[name="sendinblue_api_key_source"]').val();
		if(apiType == 'default'){
			apiKey = false;
		}else{
			apiKey = parent.find('[name="sendinblue_api_key"]').val();
		}
		let dataSendinblueList = {
			action: 'piotnetforms_sendinblue_get_list',
			apiKey: apiKey
		}
		$.post(ajaxurl, dataSendinblueList, function(response) {
			if(response){
				$('[data-piotnetforms-sendinblue-api-get-list-results]').html(response);
			}
		});
		let dataSendinblueAttribute = {
			action: 'piotnetforms_sendinblue_get_attributes',
			apiKey: apiKey
		}
		$.post(ajaxurl, dataSendinblueAttribute, function(response) {
			if(response){
				$('[data-piotnetforms-sendinblue-api-get-attributes-result]').html(response);
				$('[data-piotnetforms-sendinblue-get-list]').removeClass('loading');
			}
		});
	});
	//Convertkit
	$(document).on('click', '[data-piotnetforms-convertkit-get-data]', function(){
		$(this).addClass('loading');
		let parent = $(this).closest('[data-piotnet-controls-section-body]');
		let convertkitApi = parent.find('[name="convertkit_api_key_source"]').val();
		let convertkitApiKey;
		if(convertkitApi == 'custom'){
			convertkitApiKey = parent.find('[name="convertkit_api_key"]').val();
		}else{
			convertkitApiKey = false;
		}
		let convertkitData = {
			action: 'piotnetforms_convertkit_get_form',
			api_key: convertkitApiKey
		}
		$.post(ajaxurl, convertkitData, function(response) {
			if(response){
				$('[data-piotnetforms-convertkit-get-data-results]').html(response);
				$('[data-piotnetforms-convertkit-get-data]').removeClass('loading');
			}
		});
		let convertkitField = {
			action: 'piotnetforms_convertkit_get_fields',
			api_key: convertkitApiKey
		}
		$.post(ajaxurl, convertkitField, function(response) {
			if(response){
				$('[data-piotnetforms-convertkit-fields]').html(response);
			}
		});
	});
	//Mailchimp
	$(document).on('click', '[data-piotnetforms-mailchimp-get-data-list]', function(){
		...
		var data = {
			'action': 'piotnetforms_mailchimp_select_list',
			'api': api_key,
		};
		$.post(ajaxurl, data, function(response) {
			if(response){
	...
	$(document).on('click', '[data-piotnetforms-mailchimp-get-group-and-field]', function(){
		...
		var data_fields = {
			'action': 'piotnetforms_mailchimp_merge_fields',
			'api': api_key,
			'list_id': listId
		};
		var data_groups = {
			'action': 'piotnetforms_mailchimp_get_groups',
			'api': api_key,
			'list_id': listId
		}
	...
	//SendGrid
	$(document).on('click', '[data-piotnetforms-twilio-sendgrid-get-data-list]', function(){
		$(this).attr('disabled', 'disabled');
		$(this).addClass('loading');
		const $parent = $(this).closest('[data-piotnet-controls-section-body]');
		var apiKey = $parent.find('[name="twilio_sendgrid_api_key"]').val();
		var dataSendgrid = {
			'action': 'piotnetforms_twilio_sendgrid_get_list',
			'api': apiKey
		}
		$.post(ajaxurl, dataSendgrid, function(response) {
			if(response){
				$('[data-piotnetforms-twilio-sendgrid-get-data-list-results]').html(response);
				$('[data-piotnetforms-twilio-sendgrid-get-data-list]').removeClass('loading');
			}
		});
	});
	...
	$(document).on('click', '[data-piotnet-mailpoet-get-custom-fields]', function(){
		$(this).addClass('loading');
		var data = {
			action: 'piotnetforms_mailpoet_get_custom_fields'
		}
		$.post(ajaxurl, data, function(response) {
			if(response){
	...
	$(document).on('click', '[data-piotnetforms-zohocrm-get-tag-name]', function(){
		...
		module = module == 'custom' ? $parent.find('[name="zohocrm_custom_module"]').val() : module;
		var zoho_data = {
			action: 'zoho_get_tag_name_piotnetforms',
			module: module
		}
