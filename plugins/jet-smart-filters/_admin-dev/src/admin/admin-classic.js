(function ($) {
	...
	var JetSmartFiltersAdmin = {
		...
		init: function () {
			...
			$('#jet-smart-filters-admin-mode-switcher').on('click', self.adminModeSwitch);
		...
		adminModeSwitch: function () {
			...
			$.ajax({
				type: 'POST',
				url: JetSmartFiltersAdminData.urls.endpoints.AdminModeSwitch,
				data: { mode: 'modern' },
				beforeSend: function (jqXHR) {
					jqXHR.setRequestHeader('X-WP-Nonce', JetSmartFiltersAdminData.nonce);
				},
				success: function (responce) {
					window.location = JetSmartFiltersAdminData.urls.admin + 'admin.php?page=jet-smart-filters';
				}
			});
		},
		...
		reindexFilters: function () {
			...
			$.ajax({
				url: JetSmartFiltersAdminData.urls.ajaxurl,
				type: 'POST',
				dataType: 'json',
				data: {
