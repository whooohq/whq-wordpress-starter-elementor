...
class Main_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$desc_allowed = jet_menu()->settings_manager->options_manager->get_option( 'jet-show-top-menu-desc', 'true' );
		...
		$sub_desc_allowed = jet_menu()->settings_manager->options_manager->get_option( 'jet-show-sub-menu-desc', 'true' );
		...
		if ( in_array( 'menu-item-has-children', $item->classes ) || $this->is_mega_enabled( $item->ID ) ) {
			...
			switch ( $arrow_icon_type ) {
				case 'icon':
					$default_arrow = ( 'top' === $level ) ? 'fa-angle-down' : 'fa-angle-right';
					$arrow_icon    = jet_menu()->settings_manager->options_manager->get_option( 'jet-menu-' . $level . '-arrow', $default_arrow );
					$arrow         = jet_menu_tools()->get_dropdown_arrow_icon_html( $arrow_icon );
					break;
				case 'svg':
					$arrow_icon    = jet_menu()->settings_manager->options_manager->get_option( 'jet-menu-' . $level . '-arrow-svg', false );
					$arrow         = jet_menu_tools()->get_dropdown_arrow_svg_html( $arrow_icon );
					break;
			}
		}
		...
		if ( $this->is_mega_enabled( $item->ID ) && ! $is_elementor ) {
			...
			$ajax_loading = jet_menu()->settings_manager->options_manager->get_option( 'jet-menu-mega-ajax-loading', false );
